...
//   val fs: FS = new GoogleStorageFS(None, None)

//   def withExecuteContext[T](timer: ExecutionTimer): (ExecuteContext => T) => T =
//     ExecuteContext.scoped(tmpdir, tmpdir, null, fs, timer, null, theHailClassLoader, this.references, flags, new BackendContext {
