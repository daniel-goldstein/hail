...
object GoogleStorageFS {
  ...
  def parseUrl(filename: String): GoogleStorageFSURL = {
    val colonIndex = filename.indexOf(":")
    if (colonIndex == -1)
      throw new IllegalArgumentException(s"Invalid GCS URI: $filename")

    val scheme = filename.substring(0, colonIndex)
    if (scheme != "gs")
      throw new IllegalArgumentException(s"Invalid scheme, expected gs: $scheme")

    var rest = filename.substring(colonIndex + 1)
    if (!rest.startsWith("//"))
      throw new IllegalArgumentException(s"GCS URI must be of the form: gs://bucket/path, found $filename")

    rest = rest.substring(2)
    rest.split("/", 2) match {
      case Array(bucket, path) => GoogleStorageFSURL(bucket, path)
      case _ => throw new IllegalArgumentException(s"GCS URI must have a bucket and a path, found $filename")
    }
  }
}
...
class GoogleStorageFS(
  ...
  def listStatus(filename: String): Array[FileStatus] = retryTransientErrors {
    ...
    blobs.getValues.iterator.asScala
      .filter(b => b.getName != url.path) // elide directory markers created by Hadoop
