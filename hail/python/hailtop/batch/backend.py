...
class LocalBackend(Backend[None]):
    ...
    def _run(self,
        ...
        requester_pays_project_json = hailtop.json.dumps(batch.requester_pays_project)
        ...
        def copy_input(job, r):
            if isinstance(r, resource.InputResourceFile):
                if r not in copied_input_resource_files:
                    ...
                    if input_scheme != '':
                        transfers = hailtop.json.dumps([{"from": r._input_path, "to": r._get_path(tmpdir)}])
        ...
        try:
            ...
            if input_transfer_dicts:
                input_transfers = hailtop.json.dumps(input_transfer_dicts)
                code = new_code_block()
            ...
            for job in batch._jobs:
                ...
                if output_transfer_dicts:
                    output_transfers = hailtop.json.dumps(output_transfer_dicts)
