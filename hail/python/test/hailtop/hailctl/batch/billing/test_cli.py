...
def test_add_user(runner: CliRunner):
    res = runner.invoke(cli.app, ['add-user', 'user-foo', 'project-bar'], catch_exceptions=False)
    assert res.stdout.strip() == 'Added user user-foo to billing project project-bar'
