...
parser.add_argument(
    '-d',
    required=True,
    choices=['eqtl_associations', 'eqtl_covariates', 'genes', 'transcripts', 'exons', 'junctions'],
    help='GTEx dataset to load.',
)
...
ht_subject_phenotypes = hl.import_table(
    EXTRACT_BUCKET + 'GTEx/v7/GTEx_subject_phenotypes.v7.tsv.bgz',
    key='SUBJID',
    missing='',
    types={'SEX': hl.tint, 'DTHHRDY': hl.tint},
)
ht_subject_phenotypes = ht_subject_phenotypes.rename(
    {
        'SUBJID': 'subject_id',
        'SEX': 'subject_sex',
...
ht_sample_attributes = hl.import_table(
    EXTRACT_BUCKET + 'GTEx/v7/GTEx_sample_attributes.v7.tsv.bgz', key='SAMPID', missing=''
)
...
ht_sample_attributes = ht_sample_attributes.rename(
    {
        'SAMPID': 'sample_id',
    }
)
ht_sample_attributes = ht_sample_attributes.annotate(
    subject_id=hl.delimit(ht_sample_attributes['sample_id'].split('-')[:2], '-')
)
...
if dataset == 'genes':
    ...
    ht_genes = import_gtf(path=EXTRACT_BUCKET + 'GTEx/v7/GTEx_genes.v7.GRCh37.gtf.bgz', reference_genome='GRCh37')
    ht_genes = ht_genes.filter(ht_genes['feature'] == 'gene')
    ht_genes = ht_genes.key_by(ht_genes['gene_id'])
    ht_genes = ht_genes.select(
        'interval', 'strand', 'gene_name', 'havana_gene', 'gene_type', 'gene_status', 'level', 'tag'
    )
    ...
    ht_transcripts = import_gtf(
        path=EXTRACT_BUCKET + 'GTEx/v7/GTEx_transcripts.v7.GRCh37.gtf.bgz', reference_genome='GRCh37'
    )
    ...
    ht_exons = import_gtf(path=EXTRACT_BUCKET + 'GTEx/v7/GTEx_genes.v7.GRCh37.gtf.bgz', reference_genome='GRCh37')
    ...
    mt = mt.transmute_rows(
        junction_interval=hl.locus_interval(
            mt['chr_start_end'][0],
