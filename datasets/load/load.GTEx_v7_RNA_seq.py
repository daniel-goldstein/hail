...
ht_samples = hl.import_table(
    f'{raw_data_root}/GTEx_v7_sample_attributes.tsv.bgz', impute=True, key='SAMPID', missing=''
)
...
ht_subjects = hl.import_table(f'{raw_data_root}/GTEx_v7_subject_phenotypes.tsv.bgz', missing='', key='SUBJID')
ht_subjects = ht_subjects.rename({'SUBJID': 'subject_id', 'AGE': 'age_range'})
ht_subjects = ht_subjects.annotate(
    is_female=ht_subjects['SEX'] == '2',
    death_classification_hardy_scale=(
        hl.case()
...
ht_genes = hl.import_table(
    f'{raw_data_root}/GTEx_v7_gencode_v19_patched_contigs_genes.gtf.bgz',
    comment='#',
    no_header=True,
    missing='.',
...
ht_genes = ht_genes.transmute(
    **{x: hl.or_missing(ht_genes['attribute'].contains(x), ht_genes['attribute'][x]) for x in attributes if x}
)

ht_genes = ht_genes.annotate(
    gene_interval=hl.locus_interval(
        ht_genes['seqname'], ht_genes['start'], ht_genes['end'] + 1, reference_genome='GRCh37'
    )
)
...
mt = mt.annotate_globals(
    metadata=hl.struct(
        name=name, version=version, reference_genome=build, n_rows=n_rows, n_cols=n_cols, n_partitions=n_partitions
    )
)
...
mt = hl.import_matrix_table(
    f'{raw_data_root}/GTEx_v7_RNA_seq_gene_TPMs.tsv.bgz',
    row_fields={'Name': hl.tstr, 'Description': hl.tstr},
    row_key='Name',
    entry_type=hl.tstr,
    missing=' ',
...
mt = hl.import_matrix_table(
    f'{raw_data_root}/GTEx_v7_RNA_seq_junction_read_counts.tsv.bgz',
...
mt = mt.annotate_rows(
    junction_interval=hl.locus_interval(
        mt['_split'][0], hl.int(mt['_split'][1]), hl.int(mt['_split'][2]) + 1, reference_genome='GRCh37'
    )
)
