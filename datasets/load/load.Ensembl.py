...
parser.add_argument(
    '-d', required=True, choices=['dna', 'lcr', 'cdna', 'cds', 'ncrna', 'pep'], help='Ensembl sequence dataset to load.'
)
...
if args.d == 'dna':
    ...
    ht = hl.import_table(
        'gs://hail-datasets-extracted-data/Ensembl/{n}.{v}.{rg}.tsv.bgz'.format(n=name, v=version, rg=reference_genome),
...
if args.d in set(['cdna', 'cds', 'ncrna']):
    ...
    ht = hl.import_table(
        'gs://hail-datasets-extracted-data/Ensembl/Ensembl_{0}_regions.{1}.{2}.tsv.bgz'.format(
            args.d, version, reference_genome
        ),
        types={'start': hl.tint, 'end': hl.tint},
    )
    ...
    ht = ht.select(
        'interval', 'strand', 'seqtype', 'status', 'transcript_id', 'transcript_biotype', 'gene_id', 'gene_biotype'
    )
    ht = ht.key_by('interval')
...
ht = ht.annotate_globals(
    metadata=hl.struct(
        name=name, version=version, reference_genome=reference_genome, n_rows=n_rows, n_partitions=n_partitions
    )
)
ht.describe()
