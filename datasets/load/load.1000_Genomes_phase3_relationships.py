...
ht_relationships = ht_relationships.rename(
    {
        'Family ID': 'family_id',
        'Individual ID': 's',
...
ht_relationships = ht_relationships.annotate(
    paternal_id=hl.or_missing(ht_relationships['paternal_id'] != '0', ht_relationships['paternal_id']),
    maternal_id=hl.or_missing(ht_relationships['maternal_id'] != '0', ht_relationships['maternal_id']),
    relationship_role=hl.cond(
        ht_relationships['relationship_role'] == 'unrel', 'unrelated', ht_relationships['relationship_role']
    ),
    sibling_ids=hl.or_missing(
        ht_relationships['sibling_ids'] == '0', hl.map(lambda x: x.strip(), ht_relationships['sibling_ids'].split(','))
    ),
...
ht_relationships = ht_relationships.select(
    'family_id',
    'relationship_role',
    'maternal_id',
    'paternal_id',
...
ht_relationships = ht_relationships.annotate_globals(
    metadata=hl.struct(name='1000_Genomes_phase3_sample_relationships', n_rows=n_rows, n_partitions=n_partitions)
)
