...
if reference_genome == 'GRCh37':
    mt = hl.import_vcf(
        'gs://hail-datasets-extracted-data/dbSNP/{n}.{v}.{rg}.vcf.bgz'.format(n=name, v=version, rg=reference_genome),
...
mt_split = mt_split.annotate_rows(
    info=mt_split['info'].annotate(
        CAF=mt_split['info']['CAF'][mt_split['a_index'] - 1], TOPMED=mt_split['info']['TOPMED'][mt_split['a_index'] - 1]
    )
)
