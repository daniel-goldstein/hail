...
if args.d == 'scores':
    name = 'GERP_scores'
    ht = hl.import_table(
        'gs://hail-datasets-extracted-data/GERP++/GERP++_scores.hg19.tsv.bgz',
...
if args.d == 'elements':
    name = 'GERP_elements'
    ht = hl.import_table(
        'gs://hail-datasets-extracted-data/GERP++/GERP++_elements.hg19.tsv.bgz',
        types={'start': hl.tint, 'end': hl.tint, 'S': hl.tfloat, 'p_value': hl.tfloat},
    )
    ht = ht.annotate(
        interval=hl.interval(
            hl.locus(ht['chromosome'], ht['start'], 'hg19'), hl.locus(ht['chromosome'], ht['end'], 'hg19')
        )
    )
    if args.b == 'GRCh37':
        hg19.add_liftover('gs://hail-datasets-extracted-data/assemblies/hg19tob37.chain.gz', 'GRCh37')
...
ht = ht.annotate_globals(
    metadata=hl.struct(name=name, version='GERP++', reference_genome=args.b, n_rows=n_rows, n_partitions=n_partitions)
)
ht.describe()
