...
ht_samples = hl.import_table('gs://hail-datasets/raw-data/1000_genomes/samples_1kg.tsv', key='sample')

ht_map = hl.import_table(
    'gs://hail-datasets/raw-data/1000_genomes/genetic_map_b37.tsv',
...
mt = hl.import_vcf(
    'gs://hail-datasets/raw-data/1000_genomes/ALL.chr*.phase3_shapeit2_mvncall_integrated_v5a.20130502.genotypes.vcf.bgz',
    min_partitions=250,
)
...
mt_split = mt_split.annotate_rows(
    cm_position=ht_map[mt_split.locus].cm_position,
...
mt_split.describe()
mt_split = mt_split.repartition(500)
