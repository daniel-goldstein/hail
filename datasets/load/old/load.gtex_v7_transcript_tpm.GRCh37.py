...
ht_transcripts = hl.import_table(
    'gs://hail-datasets/raw-data/gtex/v7/reference/gencode.v19.transcripts.patched_contigs.gtf',
    comment='#',
    no_header=True,
...
ht_transcripts = ht_transcripts.annotate(
    **{x: hl.or_missing(ht_transcripts.attributes.contains(x), ht_transcripts.attributes[x]) for x in attribute_cols}
)
...
mt = hl.import_matrix_table(
    'gs://hail-datasets/raw-data/gtex/v7/rna-seq/processed/GTEx_Analysis_2016-01-15_v7_RSEMv1.2.22_transcript_tpm.tsv.bgz',
...
mt = mt.rename({'x': 'TPM'})
mt = mt.drop(mt.col_id)

mt = mt.annotate_cols(**ht_samples[mt.sample_id])
mt = mt.annotate_rows(**ht_transcripts[mt.transcript_id])

mt.describe()
mt.write('gs://hail-datasets/hail-data/gtex_v7_transcript_tpm.GRCh37.mt', overwrite=True)
