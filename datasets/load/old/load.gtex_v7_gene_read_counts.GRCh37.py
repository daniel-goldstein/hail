...
ht_samples = hl.import_table(
    'gs://hail-datasets/raw-data/gtex/v7/annotations/GTEx_v7_Annotations.SampleAttributesDS.txt',
    key='SAMPID',
    missing='',
)
...
ht_genes = hl.import_table(
    'gs://hail-datasets/raw-data/gtex/v7/reference/gencode.v19.genes.v7.patched_contigs.gtf',
    comment='#',
    no_header=True,
    types={'f3': hl.tint, 'f4': hl.tint},
    missing='.',
    min_partitions=12,
)
...
ht_genes = ht_genes.annotate(
    interval=hl.interval(
        hl.locus(ht_genes.contig, ht_genes.start, 'GRCh37'), hl.locus(ht_genes.contig, ht_genes.end + 1, 'GRCh37')
    )
)
ht_genes = ht_genes.annotate(
    attributes=hl.dict(
        hl.map(
            ...
            ht_genes.attributes.split('; '),
        )
    )
)
attribute_cols = list(ht_genes.aggregate(hl.set(hl.flatten(hl.agg.collect(ht_genes.attributes.keys())))))
ht_genes = ht_genes.annotate(
    **{x: hl.or_missing(ht_genes.attributes.contains(x), ht_genes.attributes[x]) for x in attribute_cols}
)
ht_genes = ht_genes.select(
    *(['gene_id', 'interval', 'gene_type', 'strand', 'annotation_source', 'havana_gene', 'gene_status', 'tag'])
)
ht_genes = ht_genes.rename({'havana_gene': 'havana_gene_id'})
ht_genes = ht_genes.key_by(ht_genes.gene_id)

mt = hl.import_matrix_table(
    'gs://hail-datasets/raw-data/gtex/v7/rna-seq/processed/GTEx_Analysis_2016-01-15_v7_RNASeQCv1.1.8_gene_reads.tsv.bgz',
